...
def test(args, model, test_loader):
    ...
    with open('log.txt', "a") as file:
        save_name = args.pretrained_path.split('/')[-2] + '_'
        save_name += args.pretrained_path.split('/')[-1][:-4]
        file.write('%s  AUC:%.4f' %  (save_name, auroc_avg) + "\n")
...
def main():
    ...
    parser.add_argument("--prefix_last", type=str, default='')
    ...
    parser.add_argument("--learning_rate", default=8e-3, type=float,
                        help="The initial learning rate for SGD.")                
    ...
    parser.add_argument("--dataset_path", default='./MIMIC-DATA-Final/COVID-19_and_ChestX-ray14/CXR8/images/images_all/', type=str)
    parser.add_argument("--dataset_split_path", default='./DatasetsSplits/NIH_ChestX-ray/', type=str)
    ...
    if args.stage == 'train':
        args.prefix += args.pretrained_path.split('/')[-2:-1][0] + '/'
        args.last = args.pretrained_path.split('-')[-1][:-4]
        print(args.prefix+args.last)
    ...
    try:
        ...
        with open('log.txt', "a") as file:
            save_name = args.pretrained_path.split('/')[-2] + '_'
            save_name += args.pretrained_path.split('/')[-1][:-4]
            file.write('%s is None during %s' %  (save_name, args.stage) + "\n")
        return 0
