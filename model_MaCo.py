...
class MaCo(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, lam=0.9, T=0.07, SFT=1.0, d=16, d2=16, cet=1.0):
        super().__init__()
        ...
        # self.mask_token_fixed = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim), requires_grad=False)
        ...
        self.pos_weight = nn.Linear(num_patches, 1)
    ...
    def init_pos_weight(self):
        ...
        self.pos_weight.weight = nn.Parameter(init_weight)
        # self.pos_weight_text.weight = nn.Parameter(init_weight)
    ...
    def forward_img_encoder_nomask(self, x, mask_ratio):
        x = self.patch_embed(x)
        # add pos embed w/o cls token
        x = x + self.pos_embed[:, 1:, :]
    ...
    def forward(self, batch, mask_ratio=0.50):
        ...
        x_ = torch.cat([ones_sample[:, 1:, :], mask_token_fixed], dim=1)  # no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, ones_sample.shape[2])).mean(dim=-1) # unshuffle        
        ...
        x_ = self.pos_weight(x_).type(torch.float32)
        ...
        if self.d != 0:
            x_fc = rearrange(x_, '(d1 d2) f-> d1 d2 f', d2=self.d)
            x_fc = (x_fc/self.SFT).softmax(dim=1)
            x_fc = rearrange(x_fc, 'd1 d2 f-> (d1 d2) f')
        else:
            x_fc = (x_/self.SFT).softmax(dim=0)
        
        if self.cet != 0:
            x_ce = rearrange(x_, '(d1 d2) f-> d1 d2 f', d2=self.d2)
            x_ce = (x_ce/self.cet).softmax(dim=1)
            x_ce = rearrange(x_ce, 'd1 d2 f-> (d1 d2) f')
            self.CE = torch.nn.CrossEntropyLoss(weight=x_ce.detach().squeeze(-1))
        else:
            self.CE = torch.nn.CrossEntropyLoss()

        Tem = self.Tpar
        
        logits1, logits2 = create_logits(latent_img_global, logits_report, Tem)
        gt = torch.arange(logits1.shape[0], dtype=torch.long).cuda()
        
        logits1 = logits1 * torch.exp(x_fc)
        logits2 = logits2 * torch.exp(x_fc)

        loss_c = self.CE(logits1, gt)
        loss_c += self.CE(logits2, gt)
        loss_c /= 2

        loss_c = loss_c * (1 - self.lam) #* Lambda
        loss_img = loss_img * self.lam
