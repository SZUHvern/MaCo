...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir + 'model/' + args.prefix + '/')
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            for i in model_without_ddp.state_dict():
                if i not in checkpoint['state_dict']:
                    checkpoint['state_dict'][i] = model_without_ddp.state_dict()[i]
...
def flatten_dict(d):
    assert find_max_layer(d) == 2, "Only support 2 level dict"
    new = {}
    for k, v in d.items():
