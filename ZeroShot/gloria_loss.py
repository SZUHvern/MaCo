...
def local_loss(
    ...
    for i in range(batch_size):
        ...
        accumulate_num = sum(cap_lens[:i]) 
        # TODO: remove [SEP]
        # word = words_emb[i, :, 1:words_num+1].unsqueeze(0).contiguous()    # [1, 768, 25]
        word = words_emb[accumulate_num: accumulate_num + words_num].unsqueeze(0).transpose(1, 2).contiguous()  # [1, 768, 25]
        word = word.repeat(batch_size, 1, 1)  # [48, 768, 25]
        context = img_features  # [48, 768, 19, 19]

        weiContext = attention_fn(word, context, temp1)  # [48, 768, 25], [48, 25, 19, 19]
    ...
    gt = torch.arange(batch_size, dtype=torch.long).to(similarities.device)
    loss0 = nn.CrossEntropyLoss()(similarities, gt)  # labels: arange(batch_size)
    loss1 = nn.CrossEntropyLoss()(similarities1, gt)
    return (loss0+loss1)/2
    # return similarities, similarities1