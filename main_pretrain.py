...
# --------------------------------------------------------
# References:
# MaCo: https://github.com/RL4M/MaCo-pytorch
# --------------------------------------------------------
import argparse
import datetime
import json
...
# CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29501 main_pretrain.py --SR 0 --mask_ratio 0.5 --lam 0.9 --d 16 --d2 2048 --SFT 0.1 --T 0.03 --CET 16 --batch_size 512 --epochs 51 --lr 2.5e-4 --prefix_last Macolr254e50;
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='./MIMIC-DATA-Final/MIMIC-CXR/', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--resume', default='./MaCo/resume/mae_pretrain_vit_base.pth',
    ...
    parser.add_argument('--prefix_last', default='', type=str)  #175,fixed
    ...
    parser.add_argument('--d', default=16, type=int)
    parser.add_argument('--d2', default=16, type=int)
    parser.add_argument('--SFT', default=1.0, type=float)
    parser.add_argument('--CET', default=16, type=float)
    return parser
...
def main(args):
    ...
    # define the model
    model = model_MaCo.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, T=args.T, lam=args.lam, SFT=args.SFT, d=args.d, d2=args.d2, cet=args.CET)
...
if __name__ == '__main__':
    ...
    args.prefix += ('_lam' + str(args.lam))
    args.prefix += ('T' + str(args.T))
    args.prefix += ('RV' + str(args.mask_ratio))
    args.prefix += ('SFT' + str(args.SFT))
    args.prefix += ('CET' + str(args.CET))
    args.prefix += ('B' + str(args.batch_size))
    if args.d != None:
        args.prefix += ('d' + str(args.d))
        args.prefix += ('d2' + str(args.d2))
    args.prefix += args.prefix_last
