...
def load_ms_cxr(merge=True, **kwargs):
    # loading data
    print("loading data...")
    if merge == True:
        data = merge_annotation(MS_CXR_JSON)
    else:
...
def load_covid_rural(**kwargs):
    ...
    for img in tqdm(img_list):
        ...
        # label_text_list.append("It is a contagious disease caused by a virus. "
        # "Ground-glass opacities, consolidation, thickening, "
        # "pleural effusions commonly appear in infection.")
...
def load_chexlocalize(**kwargs):
    ...
    for cxr_id in tqdm(cxr_ids):
        path = os.path.join(
            img_dir,
            cxr_id.split("_")[0], 
            cxr_id.split("_")[1], 
            "_".join(cxr_id.split("_")[2:])+ ".jpg")
        ...
        for category, elem in gt_dict[cxr_id].items():
            ...
            if np.sum(gt_mask) == 0:
                continue
            gt_mask = _resize_img(gt_mask, 224)
            mask = (gt_mask >= 1e-5).astype("uint8")
            path_list.append(Path(path))
            gtmasks_list.append(mask)
            boxes_list.append(None)
...
def merge_annotation(path_to_json, scale=224):
    ...
    for img_id, anns in coco.imgToAnns.items():
        ...
        for ann in anns:
            ...
            if label_text not in mask_dct:
                mask_dct[label_text] = mask
                bbox_dct[label_text] = [tbox]
                cats_dct[label_text] = cats[category_id]["name"]
            else:
                mask_dct[label_text] += mask
                bbox_dct[label_text].append(tbox)
                cats_dct[label_text] = cats[category_id]["name"]
...
def biovil_show(image_path, similarity_map, bboxes, save_path="", title="Similarity heatmap", gtmask=None, **kwargs):
    ...
    # _plot_image(image, axis=axes, title="Input image")
    # plt.imread(np.array(image))
